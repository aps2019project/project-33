...
public class BattleMenu {
    ...
    public static void handleDeck(Account account) throws FileNotFoundException {
        ...
        for (int i = 0; i < 5; i++) {
            Spell spell = Spell.createSpell(DeckGenerator.spellNames[7], account.getUsername());
    ...
    public ServerMassage createGame(String firstPlayerUsername, String secondPlayerUsername, String type, String mode, String chapter, String kind, int numberOfFlag) throws FileNotFoundException {
        ...
        if (type.equals("Single Player")) chooseMode(mode, chapter, kind, numberOfFlag);
        else chooseKind(kind, numberOfFlag);

        battle.runGame();
        Timer timer = new Timer(battle);
    ...
    private void chooseKind(String kind, int numberOfFlag) {
        battle.setPrize(prizeOfCustomGame);
        battle.setKind(kind);
        battle.setNumberOfFlags(numberOfFlag);
    }
...
class Timer extends Thread {
    ...
    public void run() {
        super.run();
        while (battle.getEstate().equals("none")) {
            try {
                Thread.sleep(1000);
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
            battle.setRemainTimeOfTurn(battle.getRemainTimeOfTurn() - 1);
            if (battle.getRemainTimeOfTurn() <= 0)
                battle.inputCommandLine("end turn", "sudo");
        }
    }
